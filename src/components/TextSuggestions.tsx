import { Notice, TFile, AbstractInputSuggest, App } from "obsidian";
import React, { useRef, useState, ReactPortal, useEffect } from "react";
...
import { DEFAULT_SUPPORTED_IMAGE_TYPES, TLTextShape, richTextValidator, track, useEditor } from "tldraw";
...
const TextSuggestions = track(() => {
    ...
    useEffect(() => {
        const removeSideEffectCb = editor.sideEffects.registerAfterChangeHandler('shape', (_, next, source) => {
            ...
            inputSuggest.onSelect(async (value, evt) => {
                ...
                try {
                    const asset = !(assets instanceof ObsidianTLAssetStore)
                        ? (
                            await createDataUrlImageAssetFromBlob(
                                await vaultFileToBlob(tFile),
                                {
                                    name: tFile.name,
                                }
                            )
...
class TldrawTipTapInputSuggest extends AbstractInputSuggest<InputSuggestionValue> {
    ...
    protected getSuggestions(text: string): InputSuggestionValue[] | Promise<InputSuggestionValue[]> {
        ...
        return this.app.vault.getFiles().filter(file => file.path.contains(content) && (
            imageExtenstions.some((ee) => ee === file.extension)
        )).map((e) => ({
            type: 'embed',
            value: e,
            getKey: () => e.path
